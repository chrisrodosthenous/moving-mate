...
test('review system: driver completes order, customer submits 5-star review', async ({ browser, baseURL }) => {
  ...
  const reviewText = 'Five stars — great job!';
  ...
  try {
    ...
    const orderRes = await backend.post('/api/orders', {
      ...
      data: {
        ...
        pickupLocation: { address: `RS pickup ${runId}`, lat: 35.1856, lng: 33.3823 },
        dropoffLocation: { address: `RS drop ${runId}`, lat: 35.19, lng: 33.39 },
    ...
    const countBefore = Number(
      ratingBefore.reviewCount ?? ratingBefore.totalReviews ?? ratingBefore.totalRatings ?? 0,
    );
    ...
    expect(String(reviewBody.data?.comment || '')).toContain('Five stars');
    ...
    const countAfter = Number(
      ratingAfter.reviewCount ?? ratingAfter.totalReviews ?? ratingAfter.totalRatings ?? 0,
    );
